...
func program(args []string) int {
	opt := getoptions.New()
	opt.Self("", `Parses JSON input passed from file or piped to STDIN and filters it by key or index.
	...
	opt.String("file", "", opt.Alias("f"), opt.ArgName("file"), opt.Description("JSON file to read."))
	opt.String("add", "", opt.ArgName("json input"), opt.Description("Child input to add at the current location."))
...
Indexes are positive integers.`))
	...
	if opt.Called("debug") {
		...
		jsonutils.Logger.SetOutput(os.Stderr)
	}
...
func realMain(opt *getoptions.GetOpt) error {
	...
	if !stdinIsDevice && !opt.Called("file") {
		...
		data, err = jsonutils.NewFromFile(file)
		if err != nil {
	...
	if opt.Called("add") {
		str, err := data.AddString(xpath, add)
		if err != nil {
			fmt.Fprintf(os.Stderr, "ERROR: %s\n", err)
	...
	str, err := data.GetString(include, xpath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "ERROR: %s\n", err)
		if !opt.Called("silent") {
			fmt.Fprintf(os.Stderr, ">\t%s\n", strings.ReplaceAll(str, "\n", "\n>\t"))
