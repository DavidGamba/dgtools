...
/*
Package jsonutils - Utilities to read yml files like if using xpath
*/
package jsonutils
...
var Logger = log.New(ioutil.Discard, "jsonutils ", log.LstdFlags)
...
func NewFromFile(filename string) (*JSON, error) {
	...
	return &JSON{Tree: tree}, nil
}

// NewFromReader returns a pointer to a YML object from an io.Reader.
func NewFromReader(reader io.Reader) (*JSON, error) {
	var tree interface{}
...
func NewFromString(str string) (*JSON, error) {
	var tree interface{}
...
func (js *JSON) GetString(include bool, keys []string) (string, error) {
	path := strings.Join(keys, ",")
	target, _, errPath := trees.NavigateTree(include, js.Tree, keys)
	// Check if response is a single element
	switch o := target.(type) {
	case string, int, uint, float32, float64, bool:
		if errPath != nil {
			return fmt.Sprintf("%v", o), fmt.Errorf("json path '%s' didn't return a valid string: %w", path, errPath)
		}
	...
	if errPath != nil {
		return string(out), fmt.Errorf("json path '%s' didn't return a valid string: %w", path, errPath)
	}
	if err != nil {
...
func (js *JSON) AddString(keys []string, input string) (string, error) {
	path := strings.Join(keys, ",")
	errPath := AddChildToTree(&js.Tree, &js.Tree, keys, input)
	// Check if response is a single element
	switch o := js.Tree.(type) {
	case string, int, uint, float32, float64, bool:
		if errPath != nil {
			return fmt.Sprintf("%v", o), fmt.Errorf("json path '%s' didn't return a valid string: %w", path, errPath)
		}
...
func AddChild(m *interface{}, child string) error {
	...
	var tree interface{}
	err := json.Unmarshal([]byte(child), &tree)
	if err != nil {
		return err
	}
	if m == nil {
	...
	case map[string]interface{}:
		Logger.Printf("AddChild: map[string] type")
...
func AddChildToTree(parent *interface{}, current *interface{}, p []string, child string) error {
	...
	case map[interface{}]interface{}:
		...
		return nil
	case map[string]interface{}:
		Logger.Printf("AddChildToTree: map[string] type")
		e, ok := t[p[0]]
		if !ok {
			return fmt.Errorf("%w: %s", trees.ErrMapKeyNotFound, p[0])
		}
		...
		(*current).(map[string]interface{})[p[0]] = e
		return nil
	case []interface{}:
