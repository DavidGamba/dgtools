...
To view the External Secrets Operator chain, you can use the `--es` flag.
It will pull the ExternalSecret and the SecretStore details as well.

Use the `-o yaml` option to print the raw YAML similar to what `kubectl get secret -o yaml <secret>` would print.
...
* Add completion for namespace and secrets
